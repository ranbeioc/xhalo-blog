...
## 5. Structured Review Checklist

All PRs generated by the Admin publishing pipeline must contain the structured owner review checklist inside the PR body:
- **Safety Indicators**: The PR body must explicitly state:
  - `Direct main write: no`
  - `Auto-merge: no`
  - `Owner manual review required: yes`
- **Manual Control**: The owner must perform the manual checkboxes check inside the GitHub PR UI, and manually merge or close the Pull Request.
