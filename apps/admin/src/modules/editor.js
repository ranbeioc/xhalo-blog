...
const copy = {
  zh: {
    ...
    notice: 'Publish to Test 会优先更新当前文章真实路径 {path}，写入后触发 Pages 构建。',
    edit: '编辑',
    diff: 'Diff',
    plan: 'PR 计划',
    titleField: '文章标题',
    ...
    insertMedia: '已插入媒体 Markdown 占位，请替换为媒体管理页生成的真实链接。',
    emptyDiff: '生成 Diff 后查看变更细节。',
    emptyPlan: '生成发布计划后预览操作。',
    vditorPlaceholder: '使用 Markdown 编写正文，支持粘贴、拖拽、表格、代码块和媒体链接...',
  ...
  en: {
    ...
    notice: 'Publish to Test updates the real source path {path} first and triggers a Pages build after the write.',
    edit: 'Edit',
    diff: 'Diff',
    plan: 'PR Plan',
    ...
    insertMedia: 'Inserted a media Markdown placeholder. Replace it with a real link from Media Manager.',
    emptyDiff: 'Generate a diff to inspect change details.',
    emptyPlan: 'Generate a publish plan to preview operations.',
    vditorPlaceholder: 'Write Markdown with paste, drag-drop, tables, code blocks, and media links...',
...
export function renderEditor(container, { initialPost, dashboardData }) {
    ...
    try {
      ...
      actionResultHtml = `
        <div class="alert alert-success">
          ...
          ${data.pagesDeploy?.triggered ? '<br/><span class="status-badge" data-state="ok">Pages build triggered</span>' : '<br/><span class="status-badge" data-state="warning">Pages build not triggered</span>'}
  ...
  function bindEvents() {
    ...
    container.querySelector('#btn-insert-media')?.addEventListener('click', () => {
      insertMarkdownSnippet('\n![media description](https://assets.example.com/path/to/media.png)\n');
  ...
  function mountVditor() {
    ...
    ensureVditorAssets().then((Vditor) => {
      ...
      const editor = new Vditor(host, {
        ...
        toolbar: ['headings', 'bold', 'italic', 'strike', '|', 'list', 'ordered-list', 'check', '|', 'quote', 'line', 'code', 'inline-code', '|', 'upload', 'link', 'table', '|', 'undo', 'redo', 'preview', 'fullscreen'],
        input(value) {
          post.body = value;
