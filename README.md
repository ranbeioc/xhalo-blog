...
`xhalo-blog` is the reusable open-source framework source. It provides the Admin, Worker APIs, templates, Cloudflare Pages/Workers conventions, and the Hexo/NexT initializer. It must not receive real private blog posts, upload assets, production `CNAME`, deploy targets, analytics IDs, or secrets.

Use one of two initialization modes:

- **Starter mode**: create a new private test site with the default NexT theme and a welcome test article when no historical source is available.
- **Import mode**: read an existing Hexo/NexT blog from disk and generate a private test site that preserves posts, uploads, pages, `_data`, scaffolds, scripts, NexT theme files, menu configuration, feed/search/sitemap/media-related plugin configuration, package metadata, and lockfiles.

Repository boundary for the standard route:

- `ranbeioc/xhalo-blog`: open-source framework, templates, migration tools, docs, tests, Admin, and Worker source only.
- `ranbeioc/hexo-blog`: read-only historical source for migration; never write to `main` from this framework.
- `ranbeioc/xhalo-blog-test`: private real-content test site generated by starter/import mode and deployed by Cloudflare Pages project `xhalo-blog-test`.

Both modes generate audit-friendly output for private test-site application. Import mode writes `.xhalo-import-manifest.json` and `.xhalo-import-report.md`, rewrites only environment-boundary fields such as `url`, disables legacy deploy targets, and ensures Pages `skip_render` keeps `/admin/`, `/landing/`, and `_worker.js` intact.
...
- [`docs/phase103-landing-admin-ux-e2e.md`](./docs/phase103-landing-admin-ux-e2e.md)
...
For the standalone open-source landing page, use:
